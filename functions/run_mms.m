...
function [linf_arr,ltwo_arr,ratio_inf_arr,ratio_two_arr,...
    ...
    if mms_plots
        ...
        if SS
            ...
            text((npts_arr(3)-(npts_arr(3)-npts_arr(2))/2)-57,1.0/npts_arr(3)-7.32e-3,...
                '$\mathcal{O}(\Delta z)=1$','Interpreter','latex',...
                'BackgroundColor','w','Rotation',-22,'Fontsize',14)
            set(gca,'Linewidth',1.0)
            for ii=1:length(npts_arr)-1
                text(npts_arr(ii)-0.25*npts_arr(ii),ltwou_arr(ii)-0.3*ltwou_arr(ii),...
                    num2str(oo_twou(ii),'%0.2f'),...
                    'color',...
                    'b','Fontsize',14);
            end
            text((npts_arr(3)-(npts_arr(3)-npts_arr(2))/2)-60,linfu_arr(2)+1.0e-2,...
                ['$L_{\infty}$ slope = ' num2str(mean(oo_inf_arr(2,:)),'%0.2f')],...
                'Interpreter','latex',...
                'BackgroundColor','w','Rotation',-22,'Color','r','Fontsize',14)
            % xticks([])
